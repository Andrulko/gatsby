...
```json:title=package.json "dependencies": { "gatsby": "^2.0.0", }
...
<div>
  Hello World
    ...
    <br />Repeat for every page and template that needs this layout.
...
<div>
  Hello World
    ...
    <br />#### 5. Change query to use `StaticQuery`
...
```js:title=gatsby-config.js plugins: [`gatsby-plugin-postcss`],

    <br />#### 3. Include PostCSS plugins in your `postcss.config.js` file
...
```diff:title=gatsby-node.js exports.onCreatePage = async ({ page, actions }) => { const { createPage } = actions

// page.matchPath is a special key that's used for matching pages // only on the client. if (page.path.match(/^\/app/)) {
...
- page.matchPath = "/app/*"
    ...
    <br />#### Migrating React Router client routes to @reach/router
...
```diff:title=src/utils/typography.js - const typography = new Typography(); - export default typography;

+ const typography = new Typography();
+ const { rhythm, scale } = typography;
+ export { rhythm, scale, typography as default };

    <br />### Update CSS Modules class names that use dashes
...
```javascript:title=gatsby-node.js const cssLoaderRe = /\/css-loader\// const targetFile = `.module.css`

const processRule = rule => { if (rule.oneOf) { return { ...rule, oneOf: rule.oneOf.map(processRule), } }

if (!rule.test.test(targetFile)) { return rule }

if (Array.isArray(rule.use)) { return { ...rule, use: rule.use.map(use => { if (!cssLoaderRe.test(use.loader)) { return use }
...
exports.onCreateWebpackConfig = ({ getConfig, actions }) => { const config = getConfig()

const newConfig = { ...config, module: { ...config.module, rules: config.module.rules.map(processRule), }, } actions.replaceWebpackConfig(newConfig) }

    <br />If you're using a preprocessor, you can pass in CSS Loader options when configuring [`gatsby-plugin-sass`](/packages/gatsby-plugin-sass/#how-to-use) or [`gatsby-plugin-less`](/packages/gatsby-plugin-less/#how-to-use):
...
```diff:title=package.json "dependencies": { "gatsby": "latest", "gatsby-image": "latest", "gatsby-plugin-sharp": "latest", - "gatsby-link": "^1.6.39" }

    <br />### Import graphql from Gatsby
...
```diff:title=package.json "peerDependencies": { - "gatsby": "1" + "gatsby": ">=1" }

    <br />### Change `modifyBabelrc` to `onCreateBabelConfig`
