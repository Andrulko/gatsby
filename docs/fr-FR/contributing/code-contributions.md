...
```text:title=.env.development GITHUB_API_TOKEN=YOUR_TOKEN_HERE

    <br />The `.env.development` file is ignored by git. Your token should never be committed.
...
services: db: image: mysql:5.6 container_name: sessions_db ports:
...
wordpress: image: wordpress:latest container_name: sessions_wordpress depends_on:
...
phpmyadmin: image: phpmyadmin/phpmyadmin container_name: sessions_phpmyadmin environment:
